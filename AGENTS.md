...
- **Vercel SSO protection is OFF on the `medico-consulta` project** (set by `scripts/setup-vercel.ps1` step 5). If a future Vercel account migration re-enables it, the curl-based `post-deploy-smoke` workflow and `scripts/smoke-test.ps1` will silently redirect to `/sso-api` — disable again via `vercel api /v1/projects/$PROJECT_ID --method PATCH --input <(echo '{"ssoProtection":null}')`.
- **Deploy automation lives at `scripts/setup-vercel.ps1` and `scripts/smoke-test.ps1`** (PowerShell 5.1+). Use `setup-vercel.ps1` after `vercel login` to provision the project + env vars + deploy in one shot. Use `smoke-test.ps1 -Url <url>` to run the same three checks the GitHub `post-deploy-smoke` workflow runs, locally. Both are documented in `docs/deployment.md` "Automated CLI Setup (PowerShell)".
