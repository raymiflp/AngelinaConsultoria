...
The system SHALL provide a context factory that injects a Drizzle DB client and an auth session placeholder.
...
#### Scenario: Default auth state

- GIVEN no auth middleware is wired yet
- WHEN the context is created
- THEN `session` MUST be `null` and typed as `Session | null`
...
The system MUST expose a procedure helper that rejects requests when no session is present.

#### Scenario: Authenticated request succeeds

- GIVEN the context contains a valid `session` object
- WHEN the handler executes
- THEN the response MUST be returned normally

#### Scenario: Unauthenticated request is rejected

- GIVEN the context has `session` set to `null`
