...
- `email` = `"paciente.dev@medico.local"`
...
- GIVEN a clean `medico` database with no rows in `usuarios` matching `%.dev@medico.local`
- WHEN `pnpm seed:dev` is executed
- THEN exactly one row MUST be inserted into `usuarios` with `email = 'doctor.dev@medico.local'` AND `rol = 'DOCTOR'`
- AND exactly one row MUST be inserted into `usuarios` with `email = 'paciente.dev@medico.local'` AND `rol = 'PACIENTE'`
...
- GIVEN the script has been executed once and the two `.dev@medico.local` users exist
- WHEN `pnpm seed:dev` is executed a second time
- THEN `SELECT count(*) FROM usuarios WHERE email IN ('doctor.dev@medico.local', 'paciente.dev@medico.local')` MUST return exactly 2
...
- THEN `SELECT count(*) FROM usuarios WHERE email LIKE '%.dev@medico.local'` MUST return exactly 2
- AND `SELECT count(*) FROM doctores WHERE usuario_id IN (SELECT id FROM usuarios WHERE email LIKE '%.dev@medico.local')` MUST return exactly 1
- AND `SELECT count(*) FROM pacientes WHERE usuario_id IN (SELECT id FROM usuarios WHERE email LIKE '%.dev@medico.local')` MUST return exactly 1
...
- A header line identifying the block as dev seed output (e.g. `=== medico-consulta dev seed ===`)
- `Doctor:` followed by `doctor.dev@medico.local` and `DoctorDev123!`
- `Paciente:` followed by `paciente.dev@medico.local` and `PacienteDev123!`
...
- WHEN `pnpm seed:dev` is executed
- THEN the process MUST exit with a non-zero code
- AND the stderr message MUST mention which step failed (e.g. `seed:dev: failed at usuario insert for doctor.dev@medico.local: <db error>`)
