...
The system MUST provide a Playwright test (`tests/e2e/videocall-2-users.spec.ts`) that creates two independent browser contexts via `browser.newContext()`, logs one in as the seeded doctor (`doctor.dev@angelina.local`) and the other as the seeded paciente (`paciente.dev@angelina.local`) through the existing `/login` form, and navigates both contexts to `/citas/{citaId}/llamada` for the cita produced by `pnpm seed:dev` (the cita where both users are participants, `modalidad === 'ONLINE'`, `estado === 'CONFIRMADA'`, `fechaHora` within the next 5 minutes).
