...
2. In tab 1, log in as `doctor.dev@angelina.local` with password `DoctorDev123!`. Verify the dashboard loads.
3. In tab 2 (incognito), log in as `paciente.dev@angelina.local` with password `PacienteDev123!`. Verify the dashboard loads.
...
- AND the two emails (`doctor.dev@angelina.local` and `paciente.dev@angelina.local`) MUST be assigned to specific tabs so the operator does not mix them up
